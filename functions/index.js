...
const generateUrl = (request) => {
  ...
  return url.format({
    ...
    host: FIREBASE_CONFIG.site.domain,
...
app.get('*', (req, res) => {
  ...
  if (botResult) {
    ...
    // Did you read the README? You should have set
    // functions.config().rendertron.server to where ever you deployed
    // https://github.com/GoogleChrome/rendertron on AppEngine
    ...
    // 1. function doesn't know about the public hosting as far as I can tell
    //    (docs don't offer opinion/example)
    // 2. Could fetch and return...but I found copy+paste the index.html PWA
    //    shell into file returns faster
    res.send(pwaShell());
  }
});
